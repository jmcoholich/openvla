...
## OpenVLA on Franka
#### Running Inference:
1. Start deoxies arm and gripper by SSH on the NUC from deoxys_control/deoxys:

```
./auto_scripts/auto_arm.sh config/charmander.yml
./auto_scripts/auto_gripper.sh config/charmander.yml
```
2. Start camera streams from openteach:
```
conda activate openteach
cd ~/openteach
python robot_camera.py
```
3. Run inference, change promt within file if needed:
```
conda activate openteach
cd ~/openvla
...
Note: startscript.bash will automate these tasks when ran from a terminator terminal. Instructions assume deoxy_control and openteach have already been setup.

#### Additional Files
+ ```test_camera.py``` Example subscribing to the openteach camera stream.

+ ```test_delta.py``` Example of controlling the arm via 7 DOF deltas.

+ ```test_openx.py``` Test openVLA pipeline with JACO Play data.

+ ```test_script.py``` Example passing an image through OpenVLA.

+ ```deoxys_control/``` Contains files used to control Franka via deltas.
